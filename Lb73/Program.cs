...
FillSyncList(syncList, () =>
{
    for (int i = 0; i < 1000;
    syncList.Add(rnd.Next()), Thread.Sleep(10), i++) ;});
...
{ 
    ...
    string elspsedTime = String.Format("{0:00}:{1:00}:{2:00}:{3:00}", ts.Hours, ts.Minutes, ts.Seconds, ts.Milliseconds/10);
    Console.WriteLine($"Я заполнил синхронизацию! Время: {elspsedTime}");
}
